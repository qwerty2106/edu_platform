...
//Получение пройденных уроков (для графика)
exports.getUserCourses = (req, res) => {
    const userID = req.params.userID;
    connection.query("SELECT COUNT(*) as lessons_count, DATE(created_date) as completed_date FROM completed_lessons WHERE user_id = ? GROUP BY DATE(created_date) ORDER BY completed_date DESC", [userID], (error, result) => {
        if (error) {
            console.log(error);
